...
export default function AssistantPage() {
	...
	function actionLabel(type: string) {
		...
		if (type === "delete_task") return "Delete task";
		if (type === "delete_event") return "Delete event";
		return "Action";
	}

	function isDestructive(type: string) {
		return type === "delete_task" || type === "delete_event";
	}
			...
			<div className="flex-1 overflow-y-auto space-y-3 mb-4">
				{messages.map((msg, i) => (
					<div key={i}>
						...
						{msg.action && (
							<div
								className={`max-w-2xl border rounded p-3 mt-2 ${
									isDestructive(msg.action.type)
							...
							>
								<p
									className={`text-sm font-semibold ${
										isDestructive(msg.action.type) ? "text-red-800" : ""
									}`}
								>
								...
								{!msg.confirmed && (
									<div className="flex gap-2 mt-2">
										<button
											...
											className={`text-xs text-white px-3 py-1 rounded disabled:opacity-50 ${
												isDestructive(msg.action.type) ? "bg-red-600" : "bg-gray-900"
											}`}
										>
											{confirmingIndex === i
												? "Saving..."
